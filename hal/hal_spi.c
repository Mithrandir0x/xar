...
/************************************************************************************
  ...
  Description:  Initialise USART0 for SPI.
...
{	
	...
	P3SEL |= BIT1 + BIT2 + BIT3;		// Select Peripheral functionality (SPI)
	P3DIR |= BIT1 + BIT3;				// Configure as outputs(SIMO,CLK)
...
/************************************************************************************
  ...
  Description:  Active wait loop. Please note that optimisation will have an effect
                on the waiting time.
...
//
//	DESCRIPTION:
//		Runs an idle loop for [timeout] microseconds. (1000 microseconds = 1 millisecond)
//
//  ARGUMENTS:
//      UINT16 timeout
...
void halWait(UINT16 timeout)
{
	...
	// So: 1 microsecond = 8 clock cycles
    do {
        _NOP();
    } while (--timeout);
